...
router.post('/auth/cambiar-password', (req, res) => {
  try {
    const { usuario_id, usuario, current_password, new_password } = req.body || {};
    const cleanUser = (usuario || '').toString().trim().toLowerCase();
    const cleanId = (usuario_id || '').toString().trim();
    const cleanNewPass = (new_password || '').toString().trim();
    ...
    let user = null;
    if (cleanId) {
      user = db.prepare('SELECT * FROM usuarios WHERE id = ?').get(cleanId);
    } else if (cleanUser) {
      user = db.prepare('SELECT * FROM usuarios WHERE LOWER(usuario) = ? OR LOWER(email) = ?').get(cleanUser, cleanUser);
    }
    ...
    if (current_password) {
      ...
      const isBcrypt = user.password_hash && (user.password_hash.startsWith('$2a$') || user.password_hash.startsWith('$2b$'));
      let isValid = isBcrypt ? bcrypt.compareSync(cleanCurrent, user.password_hash) : cleanCurrent === user.password_hash;
      if (!isValid) {
    ...
    console.log(`[SQLite Auth] Contraseña actualizada exitosamente para '${user.usuario}' (${user.id})`);
    res.json({ success: true, message: `Contraseña de '${user.usuario}' actualizada exitosamente en SQLite.` });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
const handleResetPassword = (req, res) => {
  try {
    const { id } = req.params;
    let targetUser = db.prepare('SELECT * FROM usuarios WHERE id = ?').get(id);
    if (!targetUser) {
      targetUser = db.prepare('SELECT * FROM usuarios WHERE LOWER(usuario) = ?').get(id.toLowerCase());
    }
    if (!targetUser) return res.status(404).json({ error: 'Usuario no encontrado' });
    ...
    db.prepare('UPDATE usuarios SET password_hash = ? WHERE id = ?').run(newHash, targetUser.id);
    console.log(`[SQLite Auth] Clave de '${targetUser.usuario}' actualizada persistentemente en SQLite`);
    res.json({ success: true, message: `Contraseña de '${targetUser.usuario}' restablecida exitosamente en SQLite` });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
