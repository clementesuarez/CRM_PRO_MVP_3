...
function ensureDefaultSeedUsers() {
  try {
    ...
    if (!adminExists) {
      insertOnly.run('usr-admin-default', 'Administrador General', 'admin', bcrypt.hashSync('admin123', 10), 'admin', 'admin@concesionaria.com', '+54 9 11 0000-1111', new Date().toISOString());
    }

    const superExists = db.prepare("SELECT * FROM usuarios WHERE usuario = 'superadmin'").get();
    if (!superExists) {
      insertOnly.run('usr-super-default', 'Super Administrador', 'superadmin', bcrypt.hashSync('superadmin123', 10), 'superadmin', 'superadmin@concesionaria.com', '+54 9 11 9999-0000', new Date().toISOString());
    } else {
      db.prepare("UPDATE usuarios SET activo = 1, nombre = 'Super Administrador' WHERE usuario = 'superadmin' AND (activo = 0 OR nombre = 'Soporte SuperAdmin')").run();
    }

    const vendExists = db.prepare("SELECT * FROM usuarios WHERE usuario = 'vendedor'").get();
    if (!vendExists) {
      insertOnly.run('usr-vend-default', 'Vendedor Salón', 'vendedor', bcrypt.hashSync('vendedor123', 10), 'vendedor', 'vendedor@concesionaria.com', '+54 9 11 2233-4455', new Date().toISOString());
    }

    const clementeExists = db.prepare("SELECT * FROM usuarios WHERE usuario = 'clemente' OR email = 'clemens_arg@hotmail.com'").get();
    if (!clementeExists) {
      insertOnly.run('usr-clemente-dev', 'Clemente Suárez (Dev)', 'clemente', bcrypt.hashSync('superadmin123', 10), 'superadmin', 'clemens_arg@hotmail.com', '+54 9 11 9999-8888', new Date().toISOString());
    }
  } catch (e) {
